...
const GET_ARCHIVED_USER_SUBMISSIONS = gql`
    query fetchArchivedSubmissions($userId: Int!) {
        submissions: fetchArchivedSubmissions (userId: $userId) {
            id
            description
            createdAt
            progress {
...
const ArchivedSubmissions = (props) => {
    ...
    return (
        <Query query={GET_ARCHIVED_USER_SUBMISSIONS} variables={{ userId }}>
            {({ loading, error, data }) => {

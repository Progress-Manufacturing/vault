...
const GET_ARCHIVED_LEAD_SUBMISSIONS = gql`
    query fetchArchivedLeadSubmissions {
        submissions: fetchArchivedLeadSubmissions {
            id
            description
            createdAt
            progress {
...
const ArchivedSubmissions = (props) => {
    ...
    return (
        <Query query={GET_ARCHIVED_LEAD_SUBMISSIONS} variables={{ userId }}>
            {({ loading, error, data }) => {

...
const GET_ACTIVE_LEAD_SUBMISSIONS = gql`
    query fetchActiveLeadSubmissions {
        submissions: fetchActiveLeadSubmissions {
            id
            description
            createdAt
            progress {
...
const ActiveSubmissions = (props) => {
    ...
    return (
        <Query query={GET_ACTIVE_LEAD_SUBMISSIONS} variables={{ userId }}>
            {({ loading, error, data }) => {

...
const GET_COMPLETED_LEAD_SUBMISSIONS = gql`
    query fetchCompletedLeadSubmissions {
        submissions: fetchCompletedLeadSubmissions {
            id
            description
            createdAt
            progress {
...
const CompletedSubmissions = (props) => {
    ...
    return (
        <Query query={GET_COMPLETED_LEAD_SUBMISSIONS} variables={{ userId }}>
            {({ loading, error, data }) => {

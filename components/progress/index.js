...
import ImprovementType from '../comments/improvementtype';
...
const GET_SUBMISSION_PROGRESS = gql`
    query submissionProgress($id: Int!) {
        ...
        improvementAreas: allImprovementAreaTypes {
            id
            name
        }
    }
    ...
    return (
        <Query query={GET_SUBMISSION_PROGRESS} variables={{ id }}>
            {({ loading, error, data }) => {           
                ...
                return (
                    <React.Fragment>
                        {data.submission.improvementAreaType === null && isAdmin &&
                            <ImprovementType types={data.improvementAreas} submissionId={id} />
                        }

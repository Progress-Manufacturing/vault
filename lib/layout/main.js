...
class Main extends Component {
  ...
  render() {
    const  { showSidebar, me, userSupervisor, admin } = this.state
    const { supervisor, lead, children } = this.props
                ...
                {(!showSidebar || size !== "small") ? (
                  <Box gridArea="appbar" background="lightBlack">
                    <Collapsible direction="horizontal" open={showSidebar}>
                      <AppBar admin={admin} supervisor={supervisor} lead={lead} menu={ (router.pathname).includes("ci") ? CiMenu.CiMenu : VaultMenu.VaultMenu} />
                    </Collapsible>
                  </Box>
                ) : (
                  <Layer

...
class Main extends Component {    
  ...
  getUserData = async () => {
    ...
    try {      
      ...
      const userData = await auth.callMSGraph(false, token, `${graphUrl}/me?$select=businessPhones,displayName,givenName,jobTitle,mail,officeLocation,surname,userPrincipalName,id,department`);
      const userSupervisor = await auth.callMSGraph(false, token, `${graphUrl}/me/manager?$select=id,businessPhones,displayName,givenName,jobTitle,mail,officeLocation,surename,userPrincipalName,department`);
      const avatar = await auth.callMSGraph(true, token, `${graphUrl}/me/photo/$value`);
      console.log(avatar);
      this.setState({
        me: userData,

...
export default class Authentication {
  ...
  authRedirectCallBack = (error, response) => {
    if (error) {
      ...
      if (response.tokenType === 'access_token') {
        ...
        console.log(`token type is: ${Object.keys(response)}`);
  ...
  // login = async () => {
  //   try {
  //         const idToken = await this.app.loginRedirect(this.applicationScope.scope);
  ...
  getToken = async () => {
    try {
      const accessToken = await this.app.acquireTokenSilent(this.applicationScope.scope);
      return accessToken
    }
    catch (error) {
      try {
        const accessToken_1 = await this.app
          .acquireTokenPopup(this.applicationScope.scope)
  ...
  acquireUserTokenSilent = (user) => {
    const auth_token = this.app.acquireTokenSilent(this.applicationScope.scope, 'https://login.microsoftonline.com/common', user).then((token) => {
